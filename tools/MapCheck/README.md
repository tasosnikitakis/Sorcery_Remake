...
| 2 layout | BFS columns on a synthetic chain, a fork and an island; a one-way link still places its room next door; on the real registry the columns are **re-derived by a different algorithm over independently-loaded data** and compared; two builds are identical; no two boxes overlap; adding a room (what the map's **N** / **I** keys do) moves none of the existing ones and the newcomer gets a clear spot; a stored position overrides auto-placement and disturbs nothing else. |
